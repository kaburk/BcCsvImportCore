...
は同時に有効化できます。
各プラグインが独自の管理画面コントローラーと独自メニューを持つため、
サービスクラスやメニュー表示が競合しません。
...
```
BcCsvImportCore                  ← 本体（単体では動作しない）
  ├── BcCsvImportSampleProducts  ← 動作確認・学習用（サンプル商品テーブル）
  ├── BcCsvImportSampleOrders    ← 受注データインポートのサンプル実装
  ├── BcCsvImportSampleOrderDetails ← 1対多の受注ヘッダー・受注明細インポート
  └── BcCsvImportBlogPosts       ← ブログ記事インポート（ブログ選択UI付き）
```

各差し替えプラグインは独自の管理画面コントローラーとメニューを持ち、
同時に有効化しても互いに競合しない。
共通設定は `config/setting.php` の `BcCsvImportCore` キーで上書きできる。
