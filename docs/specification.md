...
未完了ジョブ（`pending` / `processing` / `failed`）と、最近のジョブ履歴（`completed` / `cancelled`）を表示します。
...
最近のジョブ履歴はアコーディオンでデフォルト閉じ、新しい順に最大 20 件表示します。
...
`BcCsvImportBlogPosts/config/setting.php` で以下のキーを追加設定しています。
...
- `showImportStrategySelect`: `false`（常に追記モード）
- `showDuplicateModeSelect`: `false`（常にスキップ）
